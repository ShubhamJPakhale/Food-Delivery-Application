...
const RestaurantList = () => {
    ...
    function searchData(searchTxt,restaurants)
    {
        if(searchTxt !== "")
        {
            ...
            if(datanew.length === 0)
            {
                setErrorMessage("No Restaurant found with this name... please try another name for searching restaurant !!!")
            }
        }
        else{
            setErrorMessage("")
    ...
    if(online === false)
    {
        return(
            <>
                <h1 className="text-3xl font-medium align-middle text-center text-amber-500 m-5">You are offline... please check your internet connection...</h1>
            </>
        );
    }
    ...
    return (
        <>
            <div className="flex justify-center py-4 bg-pink-50 m-2">
                <input type={"textbox"} placeholder="Search restaurant and foods" className="border-solid border-1 border-black w-80 rounded-lg pl-3"
                    value={searchTxt} onChange={(e) => { setSearchTxt(e.target.value) }}
                ></input>
                {/* <div className="searchicon"><AiOutlineSearch/></div> */}
                <button className="ml-3 bg-blue-500 p-1 text-white rounded-xl" 
                onClick={()=> 
            ...
            <div className="flex flex-wrap items-center justify-center m-[5px] ">
                {filteredRestaurants.map((restaurantfilter) => {
                    return (
                    <Link to={"/restaurant/" + restaurantfilter.data.id} key={restaurantfilter.data.id} >
                        <Restaurantcard key={restaurantfilter.data.id} {...restaurantfilter.data}/>
                    </Link>);
                })}
